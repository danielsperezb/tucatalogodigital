...
public class ShoppingCartController {
    ...
    public ResponseEntity<ShoppingCartDTO> get(@PathVariable int shoppingCartId){
        return ResponseEntity.ok(this.shoppingCartService.get(shoppingCartId));
    }

    @PostMapping
    public ResponseEntity<ShoppingCartDTO> add(@RequestBody ShoppingCartEntity shoppingCart){
    ...
    @PutMapping
    public ResponseEntity<ShoppingCartDTO> update(@RequestBody ShoppingCartEntity shoppingCart){
        if (shoppingCart.getShoppingCartId() != null && this.shoppingCartService.exists(shoppingCart.getShoppingCartId())){
            return ResponseEntity.ok(this.shoppingCartService.save(shoppingCart));
        }
        return ResponseEntity.badRequest().build();
