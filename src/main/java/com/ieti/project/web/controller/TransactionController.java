...
public class TransactionController {
    ...
    public ResponseEntity<TransactionDTO> get(@PathVariable int transactionId){
        return ResponseEntity.ok(this.transactionService.get(transactionId));
    }

    @PostMapping
    public ResponseEntity<TransactionDTO> add(@RequestBody TransactionEntity transaction){
    ...
    @PutMapping
    public ResponseEntity<TransactionDTO> update(@RequestBody TransactionEntity transaction){
        if (transaction.getTransactionId() != null && this.transactionService.exists(transaction.getTransactionId())){
            return ResponseEntity.ok(this.transactionService.save(transaction));
        }
        return ResponseEntity.badRequest().build();
